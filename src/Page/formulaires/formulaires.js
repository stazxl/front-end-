...
import {SaveCreateToolbar} from './formulaireComponents.js'
...
export const FormulairesCreate = () => {
  ...
  const requestSettins = axios(settingsData);

  const { isLoading, isError, data, error } = useQuery({
    queryKey: ["DurationSettings"],
    queryFn: () => requestSettins,
  });
  ...
  if (isLoading) {
    ...
    //Création d'une variable settingsParams a travers l'application
    localStorage.setItem('settingsParams',JSON.stringify(data.data))
    
    return (
    <Create>
      <SimpleForm toolbar={<SaveCreateToolbar/>}>
        <Avatar
          alt="assur abo"
          ...
          <Box sx={{ display: 'flex', marginBottom: '1rem' }}>
            <ArrayInput source="abonnements" reference="Abonnements">
              <SimpleFormIterator inline>
                ...
                <NumberInput source="nombreAbonné" label="Nombre d'Abonné" />
              </SimpleFormIterator>
            </ArrayInput>
          </Box>
